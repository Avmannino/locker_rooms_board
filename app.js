 ...
 ************************************/
const NEXT_WINDOW_MINUTES = 90;         // "Next" = starts within this window
...
function parseLocker(text) {
  ...
  const simple1 = /\b(?:locker|room)\b\s*#?\s*([A-Za-z0-9\-]+)/i.exec(text);
  if (simple1) return simple1[1];

  const simple2 = /\b(?:lr|lkr)\b\s*#?\s*([A-Za-z0-9\-]+)/i.exec(text);
...
/************************************
 * FILTER into Now / Next
 ************************************/
function splitNowNext(events, now = new Date()) {
  const nextCutoff = new Date(now.getTime() + NEXT_WINDOW_MINUTES * 60_000);
  const nowList = [];
  const nextList = [];
  ...
  for (const ev of events) {
    if (ev.start <= now && now < ev.end) {
      nowList.push(ev);
    } else if (ev.start > now && ev.start <= nextCutoff) {
      nextList.push(ev);
    }
  }
 ...
 * RENDER
 ************************************/
function rinkDotClass(rink) {
  if (rink === "A") return "dot-a";
  if (rink === "B") return "dot-b";
  return "dot-c";
}
...
const roomsArray = (ev.locker || "").split(/\s*,\s*/).filter(Boolean);
const roomLabel = roomsArray.length > 1 ? "Rooms" : "Room";
room.textContent = `${roomLabel} ${ev.locker || "—"}`;
